...
export function buildKaiState(workouts, event, previousState) {
    const eventDate = new Date(event.occurredAt);
    const recentWorkouts = workouts.filter((workout) => {
        const workoutDate = new Date(workout.date);
        const diffInDays = Math.floor((eventDate.getTime() - workoutDate.getTime()) / DAY_IN_MS);
    ...
    return {
        ...
        longestStreak,
        inactiveDays,
        momentumState: decideMomentumState(recentCompletedCount, recentMissedCount, inactiveDays),
        lastKaiMessageType: previousState?.lastKaiMessageType,
        lastKaiMessageAt: previousState?.lastKaiMessageAt
    };
}
function calculateCurrentStreak(completedWorkouts) {
    ...
    for (let i = completedWorkouts.length - 1; i > 0; i -= 1) {
        ...
        const diffInDays = Math.floor((currentDate.getTime() - previousDate.getTime()) / DAY_IN_MS);
        if (diffInDays <= 2) {
            streak += 1;
            continue;
        }
...
function calculateInactiveDays(workouts, occurredAt) {
...
function decideMomentumState(recentCompletedCount, recentMissedCount, inactiveDays) {
    if (inactiveDays >= 5) {
        return "returning";
    }
