...
export function buildKaiWeeklySummary(workouts, plannedWorkouts, asOf) {
    const { weekStart, weekEnd } = getWeekRange(asOf);
    const weeklyWorkouts = workouts.filter((workout) => workout.date >= weekStart && workout.date <= weekEnd);
    const weeklyPlanned = plannedWorkouts.filter((plannedWorkout) => plannedWorkout.date >= weekStart && plannedWorkout.date <= weekEnd);
    let plannedCompletedCount = 0;
    let plannedMissedCount = 0;
    for (const plannedWorkout of weeklyPlanned) {
        const matchedWorkout = weeklyWorkouts.find((workout) => workout.date === plannedWorkout.date &&
    ...
    const completedCount = weeklyWorkouts.filter((workout) => workout.status === "completed").length;
    const missedCount = weeklyWorkouts.filter((workout) => workout.status === "missed").length;
    const remainingPlannedCount = Math.max(weeklyPlanned.length - plannedCompletedCount - plannedMissedCount, 0);
    const planAdherencePercent = weeklyPlanned.length === 0
        ? 0
...
export function buildKaiWeeklyCoachingMessage(summary, profile, nextPlannedWorkout) {
    ...
    if (summary.plannedCount > 0 && summary.plannedCompletedCount >= summary.plannedCount) {
        return {
            ...
            nextStep: nextPlannedWorkoutStep ??
    ...
    if (summary.plannedMissedCount >= 2 || summary.missedCount >= 3) {
        return {
            ...
            reason: summary.plannedMissedCount >= 2
    ...
    if (summary.completedCount >= 1) {
        return {
            ...
            reason: summary.unplannedCompletedCount > 0
    ...
    return {
        ...
        nextStep: nextPlannedWorkoutStep ?? "Plan one manageable workout for next week and finish it."
    };
}
function getWeekRange(asOf) {
