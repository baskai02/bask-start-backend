...
import { buildKaiWeeklyCoachingMessage, buildKaiWeeklySummary } from "./weekly.js";
import { buildTrainingReadinessReport } from "../exercises/readiness.js";
export function createKaiService(options) {
    return {
        getProfile(userId) {
        ...
        getKaiPayload(userId, asOf, profile = options.profileStore.getProfile(userId)) {
            ...
            const plannedWorkoutForDay = options.plannedWorkoutStore.findPlannedWorkout(userId, asOf);
            const nextPlannedWorkout = plannedWorkoutForDay &&
                (!recentEvent.date || recentEvent.date !== plannedWorkoutForDay.date)
                ? options.plannedWorkoutStore.findNextPlannedWorkoutAfter(userId, asOf, plannedWorkoutForDay.id)
                : planMatch.matchedPlanned
                    ? options.plannedWorkoutStore.findNextPlannedWorkoutAfter(userId, asOf, planMatch.plannedWorkout?.id)
                    : options.plannedWorkoutStore.findNextPlannedWorkoutAfter(userId, asOf, plannedWorkoutForDay?.id);
            const trainingReadiness = buildTrainingReadinessReport(userId, options.store.getWorkouts(userId), asOf, plannedWorkoutForDay?.type);
            return {
                userId,
                asOf,
                profile,
                dashboardState: deriveDashboardState(recentEvent, plannedWorkoutForDay, signals, asOf),
                ...
                nextPlannedWorkout,
                signals,
                kai: options.store.getKaiMessage(userId, asOf, profile, memory, planMatch, plannedWorkoutForDay, nextPlannedWorkout, trainingReadiness)
            };
        },
        getKaiWeeklyPayload(userId, asOf, profile = options.profileStore.getProfile(userId)) {
            ...
            const nextPlannedWorkout = findNextUnresolvedPlannedWorkout(plannedWorkouts, workouts, asOf);
            return {
                userId,
                asOf,
                profile,
                ...
                weeklySummary,
                nextPlannedWorkout,
                kai: buildKaiWeeklyCoachingMessage(weeklySummary, profile, nextPlannedWorkout)
            };
        },
        ...
        getPlanMatch(userId, asOf) {
            const recentEvent = options.store.getRecentEvent(userId, asOf);
            if (recentEvent.type === "none" ||
                !recentEvent.date ||
                !recentEvent.workoutType) {
                return { matchedPlanned: false };
            }
            const plannedWorkout = options.plannedWorkoutStore.findPlannedWorkout(userId, recentEvent.date, recentEvent.workoutType);
            return {
...
function deriveDashboardState(recentEvent, plannedWorkoutForDay, signals, asOf) {
...
function deriveTodayStatus(recentEvent, asOf) {
...
function deriveWeeklyState(summary) {
    ...
    if (summary.weekStatus === "on_track" &&
...
function findNextUnresolvedPlannedWorkout(plannedWorkouts, workouts, asOf) {
    return [...plannedWorkouts]
        .filter((plannedWorkout) => plannedWorkout.date >= asOf)
        .sort((a, b) => a.date.localeCompare(b.date))
        .find((plannedWorkout) => !workouts.some((workout) => workout.date <= asOf &&
