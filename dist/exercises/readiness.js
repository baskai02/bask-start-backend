...
const TRAINING_EFFECT_GUARDRAILS = {
    horizontal_press: {
...
export function buildTrainingReadinessReport(userId, workouts, asOf, plannedWorkoutType) {
    ...
    for (const session of completedSessions) {
        ...
        for (const sessionExercise of session.sessionExercises ?? []) {
            ...
            const unresolvedFactor = calculateUnresolvedFactor(hoursSinceSession, exercise.recoveryTimeHours);
            const unresolvedLoad = baseLoad * unresolvedFactor;
            applyMuscleContributions(muscleLoadMap, exercise.primaryMuscles, baseLoad * exercise.contributionWeights.primary, unresolvedLoad * exercise.contributionWeights.primary, hoursSinceSession, exercise.recoveryTimeHours);
            applyMuscleContributions(muscleLoadMap, exercise.secondaryMuscles, baseLoad * exercise.contributionWeights.secondary, unresolvedLoad * exercise.contributionWeights.secondary, hoursSinceSession, exercise.recoveryTimeHours);
            applyMuscleContributions(muscleLoadMap, exercise.stabilizers, baseLoad * exercise.contributionWeights.stabilizer, unresolvedLoad * exercise.contributionWeights.stabilizer, hoursSinceSession, exercise.recoveryTimeHours);
            applyPatternContribution(patternLoadMap, exercise.movementPattern, baseLoad, unresolvedLoad);
        }
    }
    ...
    const movementPatternSummary = [...patternLoadMap.entries()]
        .map(([movementPattern, accumulator]) => toPatternSummary(movementPattern, accumulator))
    ...
    const recommendations = buildExerciseRecommendations(muscleLoadSummary, movementPatternSummary, plannedWorkoutType);
    const sessionDecision = buildSessionDecision({
        plannedWorkoutType,
        overworkedMuscles,
        overworkedPatterns,
        recommendedExercises: recommendations.filter((entry) => entry.bucket === "recommended"),
        deprioritizedExercises: recommendations.filter((entry) => entry.bucket === "deprioritize")
    });
    ...
    const substitutionOptions = buildSubstitutionOptions({
        plannedWorkoutType,
        recommendedExercises: recommendations.filter((entry) => entry.bucket === "recommended"),
        candidateExercises: recommendations.filter((entry) => entry.bucket === "deprioritize" || entry.bucket === "avoid")
    });
    return {
        userId,
        asOf,
...
function buildSubstitutionOptions(input) {
    ...
    const usedPrimaryEffects = new Set();
    const builtOptions = input.candidateExercises
        ...
        const matchingReplacements = recommendedExerciseEntries
            .filter((candidate) => (candidate.trainingEffects ?? []).some((effect) => (exercise.trainingEffects ?? []).includes(effect)))
            .sort((left, right) => scoreReplacementQuality(exercise, right) - scoreReplacementQuality(exercise, left));
...
function buildSessionPlan(input) {
    ...
    if (normalizedPlanType.includes("lower")) {
        ...
        return hydrateSessionPlanBlocks(finalizeSessionPlan(normalizedPlanType, {
            sessionStyle: input.sessionDecision.status === "train_as_planned" ? "normal" : "modified",
            objective: input.sessionDecision.status === "train_modified"
            ...
            intensityGuidance: toIntensityGuidance(input.sessionDecision.intensityAdjustment),
            blocks: [
                {
                    slot: "main",
                    focus: "Quad-dominant lower-body work",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.recommendedExercises, ["quad_bias", "squat_pattern"], 2)
                },
                {
                    slot: "secondary",
                    focus: "Stable lower-body accessory work",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.recommendedExercises, ["calf_isolation", "quad_bias"], 2)
                },
                {
                    slot: "accessory",
                    focus: "Only add a third leg movement if it stays clearly away from posterior-chain fatigue",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.deprioritizedExercises, ["quad_bias", "unilateral_leg", "squat_pattern"], 2, {
            ...
            blocks: [
                {
                    ...
                    exampleExerciseIds: pickBlockExerciseIds(input.recommendedExercises, input.deprioritizedExercises, ["horizontal_press", "chest_isolation"], 2, input.overworkedMuscles)
                },
                {
                    slot: "secondary",
                    focus: "Lower-cost push accessories",
                    exampleExerciseIds: pickBlockExerciseIds(input.recommendedExercises, input.deprioritizedExercises, [
                        "lateral_delt_isolation",
                        "side_delt_bias",
                        "cable_pressdown",
                        "triceps_isolation",
                        "chest_isolation",
                        "vertical_press"
                    ], 2, input.overworkedMuscles)
                },
                {
                    slot: "accessory",
                    focus: "Hold back on the highest-overlap pressing if fatigue builds",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.deprioritizedExercises, ["cable_pressdown", "chest_isolation", "vertical_press", "horizontal_press"], 2, {
                        excludeDirectOverworkedOverlap: true,
                        excludeOffPlan: true,
                        allowFallback: false
                    })
    ...
    if (normalizedPlanType.includes("posterior")) {
        const focusMuscles = pickMuscles(["glutes", "hamstrings", "spinal_erectors"], limitMuscles);
        return hydrateSessionPlanBlocks(finalizeSessionPlan(normalizedPlanType, {
            sessionStyle: input.sessionDecision.status === "train_as_planned" ? "normal" : "modified",
            objective: input.sessionDecision.status === "train_modified"
            ...
            blocks: [
                {
                    ...
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.recommendedExercises, ["hinge_heavy", "glute_bias", "hamstring_isolation"], 2)
                },
                {
                    slot: "secondary",
                    focus: "Lower-cost posterior-chain accessory work",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.recommendedExercises, ["hamstring_isolation", "glute_bias"], 2)
                },
                {
                    slot: "accessory",
                    focus: "Skip the heaviest posterior-chain work if overlap stays high",
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.deprioritizedExercises, ["hinge_heavy", "hamstring_isolation", "glute_bias"], 2, {
                        excludeDirectOverworkedOverlap: true,
                        excludeOffPlan: true,
                        allowFallback: false
                    })
            ...
            blocks: [
                {
                    ...
                    exampleExerciseIds: pickBlockExerciseIds(input.recommendedExercises, input.deprioritizedExercises, ["horizontal_row", "vertical_pull"], 2, input.overworkedMuscles)
                },
                {
                    slot: "secondary",
                ...
                },
                {
                    ...
                    exampleExerciseIds: pickRecommendationIdsByEffect(input.deprioritizedExercises, [
                        "upper_trap_isolation",
                        "neutral_grip_curl",
                        "rear_delt_isolation",
                        "trap_isolation",
    ...
    return hydrateSessionPlanBlocks(finalizeSessionPlan(normalizedPlanType, {
        sessionStyle: input.sessionDecision.status === "train_as_planned" ? "normal" : "modified",
        objective: input.sessionDecision.status === "train_as_planned"
            ? "Train normally."
        ...
        blocks: [
            {
                ...
                exampleExerciseIds: pickRecommendationIds(input.recommendedExercises, input.recommendedExercises.map((entry) => entry.exerciseId), 2)
            },
            {
                slot: "secondary",
                focus: "Only add more work if the first block feels good",
                exampleExerciseIds: pickRecommendationIds(input.recommendedExercises.slice(2), input.recommendedExercises.slice(2).map((entry) => entry.exerciseId), 2)
            },
            {
                slot: "accessory",
                focus: "Skip the high-overlap work if fatigue rises quickly",
                exampleExerciseIds: pickRecommendationIds(input.deprioritizedExercises, input.deprioritizedExercises.map((entry) => entry.exerciseId), 2, {
                    excludeDirectOverworkedOverlap: true,
                    excludeOffPlan: true,
                    allowFallback: false
                })
...
function buildSessionDecision(input) {
    ...
    const topPullFallbacks = pickConstrainedOnPlanFallbackEntries(input.deprioritizedExercises, [
        "upper_trap_isolation",
        "neutral_grip_curl",
        "rear_delt_isolation",
        "trap_isolation",
    ...
    const topPushFallbacks = pickConstrainedOnPlanFallbackEntries(input.deprioritizedExercises, [
        "lateral_delt_isolation",
        "side_delt_bias",
        "cable_pressdown",
        "triceps_isolation",
    ...
    const hasOverworkedSignal = input.overworkedMuscles.length > 0 || input.overworkedPatterns.length > 0;
...
function calculateExerciseLoad(sessionExercise, exercise) {
    const effortMultiplier = sessionExercise.effort === "hard"
        ? 1.15
        : sessionExercise.effort === "easy"
            ? 0.85
            : 1;
    const repFactor = sessionExercise.reps / 10;
    return roundToTwoDecimals(sessionExercise.sets * repFactor * exercise.fatigueScore * effortMultiplier);
}
function calculateUnresolvedFactor(hoursSinceSession, recoveryTimeHours) {
    if (hoursSinceSession >= recoveryTimeHours) {
        return 0;
    }
    return roundToTwoDecimals(1 - hoursSinceSession / recoveryTimeHours);
}
function applyMuscleContributions(map, muscles, totalLoad, unresolvedLoad, hoursSinceSession, recoveryTimeHours) {
    for (const muscle of muscles) {
        ...
        current.maxRecoveryTimeHours = Math.max(current.maxRecoveryTimeHours, recoveryTimeHours);
        current.minHoursSinceLastWorked = Math.min(current.minHoursSinceLastWorked, hoursSinceSession);
        map.set(muscle, current);
    }
}
function applyPatternContribution(map, movementPattern, totalLoad, unresolvedLoad) {
    const current = map.get(movementPattern) ?? { totalLoad: 0, unresolvedLoad: 0 };
    current.totalLoad += totalLoad;
    current.unresolvedLoad += unresolvedLoad;
    map.set(movementPattern, current);
}
function toMuscleSummary(muscle, accumulator) {
    const hoursSinceLastWorked = accumulator.minHoursSinceLastWorked === NO_HISTORY_HOURS
        ? undefined
        : accumulator.minHoursSinceLastWorked;
    const hoursUntilRecovered = Math.max(accumulator.maxRecoveryTimeHours - (hoursSinceLastWorked ?? 0), 0);
    const riskScore = roundToTwoDecimals(accumulator.unresolvedLoad * 1 + accumulator.totalLoad * 0.2);
    return {
        muscle,
        ...
        hoursUntilRecovered,
        recoveryState: determineRecoveryState(accumulator.unresolvedLoad, riskScore, hoursUntilRecovered),
        riskScore
    };
}
function toPatternSummary(movementPattern, accumulator) {
    const riskScore = accumulator.unresolvedLoad * 0.9 + accumulator.totalLoad * 0.2;
    return {
        movementPattern,
        totalLoad: roundToTwoDecimals(accumulator.totalLoad),
        unresolvedLoad: roundToTwoDecimals(accumulator.unresolvedLoad),
        recoveryState: determineRecoveryState(accumulator.unresolvedLoad, riskScore, accumulator.unresolvedLoad)
    };
}
function determineRecoveryState(unresolvedLoad, riskScore, hoursUntilRecovered) {
    if (unresolvedLoad >= 22 || riskScore >= 34) {
        return "overworked";
    }
    if (hoursUntilRecovered >= 48 &&
...
function buildExerciseRecommendations(muscleLoadSummary, movementPatternSummary, plannedWorkoutType) {
    const muscleMap = new Map(muscleLoadSummary.map((entry) => [entry.muscle, entry]));
    const patternMap = new Map(movementPatternSummary.map((entry) => [entry.movementPattern, entry]));
    return getExerciseLibrary()
        ...
        const fallbackTier = bucket !== "avoid" && planFit.relevance === "on_plan"
...
function getMusclePenalty(summary, role) {
    const roleMultiplier = role === "primary" ? 1 : role === "secondary" ? 0.5 : 0.25;
    if (summary.recoveryState === "overworked") {
...
function formatMuscleNames(muscles) {
...
function formatExerciseNames(exercises) {
...
function toFallbackTier(tolerance) {
    if (tolerance === "green") {
        return "best";
    }
    if (tolerance === "yellow") {
...
function toVolumeGuidance(adjustment) {
...
function toIntensityGuidance(adjustment) {
...
function buildSubstitutionReason(exercise, swapForExerciseIds, swapReasonTags) {
...
function buildSubstitutionFrontendCopy(exercise, swapForExerciseIds, swapReasonTags) {
    ...
    const explanation = swapReasonTags.length >= 2
    ...
    return {
        ...
        explanation: swapNames.length > 1
            ? `${primarySwapName} is the cleanest swap today. ${explanation}`
            : `${primarySwapName} is the cleanest swap today. ${explanation}`
    };
}
function formatExerciseNameList(names) {
    if (names.length === 0) {
...
function formatTrainingEffect(effect) {
    if (!effect) {
        return "same-session";
    }
    return effect.replaceAll("_", " ");
}
function scoreReplacementQuality(original, replacement) {
    const sharedEffects = (replacement.trainingEffects ?? []).filter((effect) => (original.trainingEffects ?? []).includes(effect)).length;
    const fatigueAdvantage = Math.max(original.fatigueScore - replacement.fatigueScore, 0);
    const axialLoadAdvantage = Math.max(getAxialLoadScore(original) - getAxialLoadScore(replacement), 0);
    const setupAdvantage = Math.max(getSetupFrictionScore(original) - getSetupFrictionScore(replacement), 0);
    return sharedEffects * 10 + fatigueAdvantage * 3 + axialLoadAdvantage * 2 + setupAdvantage;
}
function buildSwapReasonTags(original, replacement) {
    const tags = [];
...
function formatSwapReasonTag(tag) {
...
function getAxialLoadScore(exercise) {
    let score = 0;
    ...
    if (exercise.primaryMuscles.includes("spinal_erectors") ||
...
function getSetupFrictionScore(exercise) {
    let score = 0;
    if (exercise.equipmentType === "barbell") {
        score += 3;
    }
...
function pickMuscles(preferred, limitMuscles) {
    const limited = new Set(limitMuscles);
    const picked = preferred.filter((muscle) => !limited.has(muscle));
    return picked.slice(0, 3);
}
function pickRecommendationIds(entries, preferredIds, maxCount, options) {
    ...
    return pickExerciseIds(eligibleEntries.map((entry) => entry.exerciseId), preferredIds, maxCount, options?.allowFallback ?? true);
}
function pickRecommendationIdsByEffect(entries, preferredEffects, maxCount, options) {
    const eligibleEntries = entries.filter((entry) => {
        if (options?.excludeDirectOverworkedOverlap && hasDirectOverworkedOverlap(entry)) {
            return false;
        }
    ...
    const preferredIds = eligibleEntries
        ...
        .filter((candidate) => Boolean(candidate.exercise))
        .filter((candidate) => (candidate.exercise.trainingEffects ?? []).some((effect) => preferredEffects.includes(effect)))
        .sort((left, right) => getPreferredEffectRank(left.exercise, preferredEffects) -
...
function pickBlockExerciseIds(recommendedEntries, deprioritizedEntries, preferredEffects, maxCount, overworkedMuscles) {
    const recommendedIds = pickRecommendationIdsByEffect(recommendedEntries, preferredEffects, maxCount, { allowFallback: false });
    if (recommendedIds.length > 0) {
        return recommendedIds;
    }
    return pickConstrainedOnPlanFallbackEntries(deprioritizedEntries, preferredEffects, maxCount, overworkedMuscles).map((entry) => entry.exerciseId);
}
function pickConstrainedOnPlanFallbackEntries(entries, preferredEffects, maxCount, overworkedMuscles) {
    const eligibleEntries = entries
        ...
        .filter((candidate) => Boolean(candidate.exercise))
        .filter((candidate) => Boolean(getBestMatchingEffect(candidate.exercise, preferredEffects, overworkedMuscles)));
    const preferredIds = eligibleEntries
        .sort((left, right) => {
        const leftMatch = getBestMatchingEffect(left.exercise, preferredEffects, overworkedMuscles);
        const rightMatch = getBestMatchingEffect(right.exercise, preferredEffects, overworkedMuscles);
        return (getToleranceRank(left.entry.tolerance) - getToleranceRank(right.entry.tolerance) ||
        ...
        .map((candidate) => candidate.exercise.exerciseId);
    const pickedIds = pickExerciseIds(eligibleEntries.map((candidate) => candidate.entry.exerciseId), preferredIds, maxCount, false);
    return pickedIds
        .map((exerciseId) => eligibleEntries.find((candidate) => candidate.entry.exerciseId === exerciseId))
        .map((candidate) => candidate?.entry)
        .filter((entry) => Boolean(entry));
}
function getToleranceRank(tolerance) {
    if (tolerance === "green") {
        return 0;
    }
    if (tolerance === "yellow") {
...
function getExerciseTolerance(exercise, muscleMap) {
...
function getMuscleDrivenTolerance(exercise, muscleMap) {
    if (exercise.primaryMuscles.some((muscle) => muscleMap.get(muscle)?.recoveryState === "overworked")) {
        return "red";
    }
    if (exercise.primaryMuscles.some((muscle) => muscleMap.get(muscle)?.recoveryState === "recovering") ||
        exercise.secondaryMuscles.some((muscle) => muscleMap.get(muscle)?.recoveryState === "recovering")) {
        return "yellow";
    }
    return "green";
}
function getEffectToleranceSeverity(effect, muscleMap) {
    const guardrail = TRAINING_EFFECT_GUARDRAILS[effect];
    if (!guardrail) {
        return 0;
    }
    const hasBlockedOverlap = guardrail.blockedBy.some((muscle) => muscleMap.get(muscle)?.recoveryState === "overworked");
...
function severityToTolerance(severity) {
    if (severity <= 0) {
        return "green";
    }
    if (severity === 1) {
...
function getPreferredEffectRank(exercise, preferredEffects) {
...
function getBestMatchingEffect(exercise, preferredEffects, overworkedMuscles) {
    ...
    return (exercise.trainingEffects ?? [])
        ...
        .map((effect) => {
        const guardrail = TRAINING_EFFECT_GUARDRAILS[effect];
        const blocked = (guardrail?.blockedBy ?? []).some((muscle) => overworkedSet.has(muscle));
        if (blocked) {
            return undefined;
        }
        return {
            ...
            cautionCount: (guardrail?.cautionBy ?? []).filter((muscle) => overworkedSet.has(muscle)).length
        };
    })
        .filter((match) => Boolean(match))
        .sort((left, right) => left.rank - right.rank || left.cautionCount - right.cautionCount)[0];
}
function finalizeSessionPlan(normalizedPlanType, plan) {
    ...
    if (normalizedPlanType.includes("push")) {
        return {
            ...
            coachNote: "Skip your main pressing work today. Keep only the small push accessories that stay clearly away from the most fatigued overlap."
        };
    }
    if (normalizedPlanType.includes("pull")) {
        return {
            ...
            coachNote: "Skip your main pulling work today. Keep only the lightest pull accessories that still feel clearly recoverable."
        };
    }
    if (normalizedPlanType.includes("lower") || normalizedPlanType.includes("posterior")) {
        return {
            ...
            coachNote: "Skip the main work today and keep only the lowest-fatigue accessory pieces that stay away from the biggest recovery flags."
        };
    }
    return {
        ...normalizedPlan,
        sessionStyle: "accessory_only",
        coachNote: "Skip the main work today and keep only the small accessory pieces that are easiest to recover from."
    };
}
function dedupeSessionPlanBlocks(blocks) {
...
function hydrateSessionPlanBlocks(plan, entries) {
    const entryByExerciseId = new Map(entries.map((entry) => [entry.exerciseId, entry]));
    return {
        ...plan,
...
function hasDirectOverworkedOverlap(entry) {
    return entry.reasons.some((reason) => reason.includes("still overworked") ||
        reason.includes("pattern is still overworked"));
}
function isOffPlanRecommendation(entry) {
    return entry.reasons.some((reason) => reason.startsWith("Less relevant to today's"));
}
function isOnPlanRecommendation(entry) {
    return entry.reasons.some((reason) => reason.startsWith("Fits today's") ||
        reason.startsWith("Still relevant to today's"));
}
function pickExerciseIds(sourceIds, preferredIds, maxCount, allowFallback = true) {
...
function scoreExercisePlanFit(exercise, plannedWorkoutType) {
    ...
    const isLowerBodyPlan = normalizedType.includes("lower") || normalizedType.includes("legs");
