...
import { parseAsOfDate, parsePlannedWorkoutInput, parseProfileInput, parseUserIdBody, parseWorkoutCompletionInput, parseWorkoutMissedInput, ValidationError } from "./api/validation.js";
...
import { KAI_SYSTEM_PROMPT, KAI_USER_PROMPT_TEMPLATE } from "./kai/agent-prompt.js";
...
const profileStorageFilePath = fileURLToPath(new URL("../data/profiles.json", import.meta.url));
const memoryStorageFilePath = fileURLToPath(new URL("../data/kai-memory.json", import.meta.url));
const plannedWorkoutStorageFilePath = fileURLToPath(new URL("../data/planned-workouts.json", import.meta.url));
...
const server = createServer(async (request, response) => {
    try {
        ...
        if (request.method === "POST" && url.pathname === "/workouts/completed") {
            ...
            const matchedPlannedWorkout = plannedWorkoutStore.findPlannedWorkout(input.userId, input.date, input.type);
            sendJson(response, 200, {
        ...
        if (request.method === "POST" && url.pathname === "/profiles") {
            ...
            const memory = kaiService.getFreshMemory(profile.userId, todayAsDateString(), profile);
            sendJson(response, 200, {
                ...buildProfileWriteResponse(profile.userId, todayAsDateString(), profile, memory)
            }, request);
            return;
        }
        if (request.method === "GET" && url.pathname.startsWith("/users/")) {
            ...
            if (parts.length === 3 && parts[2] === "signals") {
                const userId = parts[1];
                const asOf = parseAsOfDate(url.searchParams.get("asOf"), todayAsDateString());
            ...
            if (parts.length === 3 && parts[2] === "training-readiness") {
                ...
                sendJson(response, 200, buildTrainingReadinessReport(userId, store.getWorkouts(userId), asOf, plannedWorkoutForDay?.type), request);
                return;
            }
        ...
        if (request.method === "POST" && url.pathname.startsWith("/users/")) {
            ...
            if (parts.length === 4 && parts[2] === "test-scenarios") {
                ...
                sendJson(response, 200, {
                    ...
                    trainingReadiness: buildTrainingReadinessReport(userId, store.getWorkouts(userId), seeded.asOf, plannedWorkoutStore.findPlannedWorkout(userId, seeded.asOf)?.type)
                }, request);
                return;
            }
            if (parts.length === 3 && parts[2] === "profile") {
                ...
                const memory = kaiService.getFreshMemory(userId, todayAsDateString(), savedProfile);
                sendJson(response, 200, {
                    ...buildProfileWriteResponse(userId, todayAsDateString(), savedProfile, memory)
                }, request);
                return;
            }
            if (parts.length === 4 &&
                ...
                const matchedPlannedWorkout = plannedWorkoutStore.findPlannedWorkout(userId, input.date, input.type);
                sendJson(response, 200, {
                    ...buildWorkoutWriteResponse({
                        actionMessage: "Completed workout recorded.",
                        userId,
            ...
            if (parts.length === 3 && parts[2] === "planned-workouts") {
                const userId = parts[1];
                const body = await readJsonBody(request);
                const input = parsePlannedWorkoutInput(body, userId);
        ...
        const message = error instanceof Error ? error.message : "Something went wrong in the server.";
...
function sendHtml(response, statusCode, html, request) {
    response.writeHead(statusCode, {
        ...buildCorsHeaders(request),
...
function sendEmpty(response, statusCode, request) {
...
function buildCorsHeaders(request) {
    ...
    const allowOrigin = corsAllowedOrigins === "*"
        ? "*"
...
function buildKaiPayload(userId, asOf, profile = kaiService.getProfile(userId), _memory) {
...
function buildProfileWriteResponse(userId, asOf, profile, memory) {
    return {
...
function buildResetWriteResponse(userId, asOf, profile = profileStore.getProfile(userId), memory = kaiService.getFreshMemory(userId, asOf, profile)) {
    return {
...
function buildFrontendReadinessResponse(userId, asOf) {
    const plannedWorkoutForDay = plannedWorkoutStore.findPlannedWorkout(userId, asOf);
    const trainingReadiness = buildTrainingReadinessReport(userId, store.getWorkouts(userId), asOf, plannedWorkoutForDay?.type);
