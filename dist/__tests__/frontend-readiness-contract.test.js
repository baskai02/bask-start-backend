...
test("today-readiness contract stays stable for a normal planned session", () => {
    ...
    const trainingReadiness = buildTrainingReadinessReport("user_1", store.getWorkouts("user_1"), seeded.asOf, plannedWorkout?.type);
    const response = buildFrontendTrainingReadinessResponse("user_1", seeded.asOf, trainingReadiness);
    assert.deepEqual({
...
test("today-readiness contract stays stable for pull accessory-only fallback", () => {
    ...
    assert.deepEqual({
        ...
        primaryBlock: response.sessionPlan.blocks.find((block) => (block.exampleExercises?.length ?? block.exampleExerciseIds.length) > 0),
...
test("today-readiness contract stays stable for posterior substitution copy", () => {
    ...
    assert.deepEqual(response.substitutionOptions.slice(0, 2), [
        ...
        },
        {
            ...
            swapForExerciseIds: ["leg_extension"],
            swapReasonTags: ["lower_fatigue", "lower_axial_load", "lower_setup_friction"],
            reason: "Preserves the unilateral leg training effect while giving you a lower-fatigue option and lower axial load today.",
            frontendCopy: {
